...
und WRITER erleichtert, Informationen asynchron auszutauschen. Der
Puffer wird in der Regel als Array mit zwei Zeigern implementiert. Einem
...
Events\cite{ftraceintroducation}. Der
...
Funktionen\cite{fgtrace}. Auch kann dieser den Stacktrace und den
...
\subsubsection{kretprobes}\label{kretprobes}}

\passthrough{\lstinline!kretprobes!}\cite{kprobes}

\hypertarget{uprobes}{%
\subsection{uprobes}\label{uprobes}}

Eine Weiterentwicklung zu den kprobes sind die uprobes. Mit diesem
können zur Laufzeit Events in eine Applikation eingebunden werden.

Wenn ein uprobes hinzugefügt werden soll, muss davor noch was gemacht
werden. Bei der Nutzung von kprobes kann ein einfacher Symbolnamen
spezifiziert werden. Aufgrund das alle Applikationen ihren eigenen
virtuellen Adressraum besitzen, haben diese auch einen anderen
Adressbasis. Beim Erzeugen eines uprobes wird das Adressoffset im
Textsegment der jeweiligen Applikation benötigt.
